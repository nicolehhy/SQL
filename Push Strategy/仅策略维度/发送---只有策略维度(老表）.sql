...
SELECT  '$date'
        ...
        ,count(distinct a.info_uid) as send_uv
        ,count(distinct a.info_lid) as send_pv
FROM
(
SELECT   info_uid
        ,info_lid
        ,CASE WHEN lower(info_lid) like '%groupdvalue%' then 'GroupDVALUE'
                WHEN lower(info_lid) like '%groupdrfm%' then 'GroupDRFM'
                WHEN lower(info_lid) like '%groupa%' then 'GroupA'
                WHEN lower(info_lid) like '%groupb%' then 'GroupB'
                WHEN lower(info_lid) like '%groupc%' then 'GroupC'
                WHEN lower(info_lid) like '%groupd%' then 'GroupD'
                WHEN lower(info_lid) like '%groupe%' then 'GroupE'
                WHEN lower(info_lid) like '%groupf%' then 'GroupF'
                WHEN lower(info_lid) like '%groupg%' then 'GroupG'
                WHEN lower(info_lid) like '%grouph%' then 'GroupH'
                WHEN lower(info_lid) like '%groupi%' then 'GroupI'
                WHEN lower(info_lid) like '%groupj%' then 'GroupJ'
                WHEN lower(info_lid) like '%groupk%' then 'GroupK'
                WHEN lower(info_lid) like '%groupl%' then 'GroupL'
                WHEN lower(info_lid) like '%groupm%' then 'GroupM' else 'other'
                END AS strategy 
        FROM
        hds.view_newserverlog_recall_new_push
        WHERE  ymd = '$date'
...
;" > push_send_strategy.txt

sed -i '/^WARN.*/d' push_send_strategy.txt
